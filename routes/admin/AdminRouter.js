...
// auth routes
// const authInstance = require('../../utils/authSettings')
...
/**
 * @description To show new login screen with google and github auth
 * @param  {} req
 * @param  {} response
 */
...
/**
 * @description To show github screen for user login
 * @param  {} req
 * @param  {} response
 */

// router.get('/auth/github', authInstance.authenticate('github'));

/**
 * @description To redirect to callback url login screen
 * @param  {} req
 * @param  {} response
 */

// router.get('/auth/github/callback', authInstance('github'), {
//     callbackUrl: settings.githubCallbackUrl,
//     callbackHandler: async(req,res,user)=>{
...
router.get('/getVendorDetails', async (req, res) => {
  // res.sendFile(path.join(__dirname, '../../views/addVendor.html'));
  try {
    const result = await adminService.GetVendorDetails()
    if (result.success) {
...
router.post('/validate', jwtMiddleware, async (req, res) => {
  ...
  try {
    const result = await adminService.ValidateUser(userdata)
    if (result.success) {
...
router.get('/getUserType', authorize, async (req, res) => {
  try {
    const result = await adminService.GetUserType()
    if (result.success) {
      res.status(200).json({ success: true, data: result.data })
    } else {
...
router.get('/getTeam', authorize, async (req, res) => {
  try {
    const result = await adminService.GetTeam()
    if (result.success) {
      res.status(200).json({ success: true, data: result.data })
    } else {
      res.status(400).json({ success: false, message: message.GET_ERROR })
...
router.put('/insertQuotationDetails', async (req, res) => {
  ...
  try {
    const result = await adminService.InsertQuotationDetails(req.body)
    if (result.success) {
      res.status(200).json({ success: true, data: result.data })
    } else {
      return res.status(400).json({ success: false, message: result.message })
...
router.get('/getQuotationDetails/:vendorId', async (req, res) => {
  ...
  try {
    const result = await adminService.GetQuotationDetails(vendorId)
    if (result.success) {
      res.status(200).json({ success: true, data: result.data })
    } else {
      return res.status(400).json({ success: false, message: result.message })
...
router.put('/updateQuotationDetails', async (req, res) => {
  ...
  try {
    const result = await adminService.UpdateQuotationDetails(req.body)
    if (result.success) {
      res.status(200).json({ success: true, data: result.data })
    } else {
      return res.status(400).json({ success: false, message: result.message })
...
router.get('/getVendorDetails/:vendorId', async (req, res) => {
  const vendorId = req.params.vendorId
  try {
    const result = await adminService.GetVendorDetailsForUpdate(vendorId)
    if (result.success) {
      // res.redirect('http:/localhost:3000/login')
      if (result.message === message.USER_ALREADY_EXISTS) {
        return res.status(409).json({ success: true, message: message.USER_ALREADY_EXISTS })
...
router.put('/updateVendorDetails', async (req, res) => {
  ...
  try {
    const result = await adminService.UpdateVendorDetails(req.body)
    if (result.success) {
      res.status(200).json({ success: true, data: result.data })
    } else {
      return res.status(400).json({ success: false, message: result.message })
